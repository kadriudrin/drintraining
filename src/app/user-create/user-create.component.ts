...
import {confirmPasswordValidator, getFormValidationErrors} from '../shared/validators/confirmPassword.validator';
...
export class UserCreateComponent implements OnInit {
  ...
  async createNewUser() {
    this.myForm.patchValue({
      profileUrl: 'https://upload.wikimedia.org/wikipedia/commons/6/6d/Windows_Settings_app_icon.png',
      full_name: `${this.myForm.value.name} ${this.myForm.value.surname}`
    });
    if (this.myForm.valid) {
      await this.userService.createUser(this.myForm.value).subscribe(res => console.log('Res: ', res), err => console.error('Err: ', err));
    } else {
    ...
    this.myForm = this.fb.group(
      {
        ...
        validator: confirmPasswordValidator('password', 'password_confirmation')
      }
    );
  }
